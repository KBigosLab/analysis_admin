
var db = require('analysis/db');
var passport = require('passport');
var GoogleStrategy = require('passport-google').Strategy;
var signature = require('cookie-signature');

exports.doLogin = function($P,userID,persist) {
  persist = persist || false;
  var sessionKey = $P.req.session.id;
  if (userID == 0) return false;
  db.user_sessions.createSession(userID,sessionKey,persist);

  var expires = persist ? new Date(Date.now()+3600*24*14*1000) : false;
  if ($P.res.cookie('loggedIn', signature.sign(sessionKey,Const.cookieSecret), {expires: expires})) {
    if ($P.browser)
      db.browsers.addBrowser($P.browser,userID);
    $P.loggedIn = true;
    $P.userID = userID;
    var info = db.users.getUser(userID);
    $P.lastName = info.lastName;
    $P.preferredName = info.preferredName;
    $P.timezone = info.timezone;
    return true;
  } else {
    return false;
  }
}


exports.init = function(app) {

  // Google authentication
  passport.use(new GoogleStrategy({
    returnURL: Const.server+'auth/google/callback',
    realm: Const.server
  }, function(identifier, profile, done) {
    done(null,{
      type: 'google',
      id:identifier,
      emails: profile.emails && profile.emails.length ? profile.emails : [],
      name: profile.name,
    });
  }));

  app.get('/auth/google', passport.authenticate('google'));
  app.get('/auth/google/callback',
  passport.authenticate('google', { successRedirect: '/auth_redirect',
                                    failureRedirect: '/auth_redirect' }));


  // User serialization
  passport.serializeUser(function(obj, done) {
    done(null, JSON.stringify(obj));
  });

  passport.deserializeUser(function(str, done) {
    done(null, JSON.parse(str));
  });

}

